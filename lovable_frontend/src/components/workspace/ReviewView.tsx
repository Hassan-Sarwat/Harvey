...
import { useAuditStore, type AuditResult, type Violation } from "@/lib/auditStore";
...
type ChatMsg =
  ...
  | { role: "assistant"; kind: "audit"; result: AuditResult };

export const ReviewView = () => {
  const [messages, setMessages] = useState<ChatMsg[]>([]);
  const [input, setInput] = useState("");
  const [pendingContract, setPendingContract] = useState<{ text: string; name: string } | null>(null);
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    setPendingContract({ text, name: file.name });
    setMessages((m) => [
      ...m,
      { role: "user", kind: "upload", fileName: file.name, preview: text.slice(0, 240) },
    ]);
    void runAudit(text, file.name);
  }, []);

  const loadSample = () => {
    setPendingContract({ text: SAMPLE_CONTRACT, name: "sample-dpa.txt" });
    setMessages((m) => [
      ...m,
      { role: "user", kind: "upload", fileName: "sample-dpa.txt", preview: SAMPLE_CONTRACT.slice(0, 240) },
    ]);
    void runAudit(SAMPLE_CONTRACT, "sample-dpa.txt");
  };

  const runAudit = async (text: string, name: string) => {
    setLoading(true);
    setMessages((m) => [...m, { role: "assistant", kind: "thinking" }]);
    try {
      const { data, error } = await supabase.functions.invoke("audit-dpa", { body: { contractText: text } });
      if (error) throw new Error(error.message || "Audit failed");
      if (data?.error) throw new Error(data.error);
      const result = data as AuditResult;
      addAudit({ fileName: name, contractText: text, result });
      setMessages((m) => [
  ...
  const sendText = () => {
    ...
    setInput("");
    setPendingContract({ text, name: "pasted-contract.txt" });
    void runAudit(text, "pasted-contract.txt");
  };
          ...
          <div className="max-w-3xl mx-auto px-6 py-4">
            <div className="rounded-2xl border border-border/80 bg-card/60 shadow-card focus-within:border-primary/60 transition-colors">
              <textarea
                ...
                placeholder="Paste contract text, or upload a DPA to audit…"
              ...
              <div className="flex items-center justify-between px-3 pb-3">
                <div className="flex items-center gap-1">
                  ...
                  <Button type="button" size="sm" variant="ghost" onClick={loadSample} className="text-muted-foreground hover:text-foreground">
            ...
            <div className="text-[10px] text-muted-foreground text-center mt-2 font-mono">
              Audits use BMW Playbook (P-##) + GDPR/BDSG (L-##) · Powered by Lovable AI
            </div>
          </div>
        </div>
      </div>
...
const Welcome = ({ onSample, onUpload }: { onSample: () => void; onUpload: () => void }) => (
  <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center pt-12">
    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-border/80 bg-card/60 text-xs text-muted-foreground mb-6">
      ...
      Gemini 2.5 Pro · Structured audit
    </div>
    <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">
      Audit a DPA in <span className="text-gradient">seconds</span>
    </h1>
    <p className="text-muted-foreground max-w-xl mx-auto mb-10">
      Drop a Data Processing Agreement and the BMW Group's AI counsel will cross-check every clause against
      the corporate playbook and German GDPR law.
    </p>
    <div className="grid sm:grid-cols-2 gap-3 max-w-xl mx-auto">
      ...
      >
        ...
        <div className="text-xs text-muted-foreground">.txt or .md works best for the demo.</div>
      </button>
      <button
        onClick={onSample}
        className="group rounded-2xl border border-border/60 gradient-card p-5 text-left hover:border-primary/60 transition-colors"
      >
        ...
        <div className="text-xs text-muted-foreground">€7.5M DPA with multiple violations.</div>
      </button>
    </div>
  </motion.div>
);
  ...
  if (m.role === "assistant" && m.kind === "thinking") {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 text-muted-foreground">
        ...
        <div className="text-sm">Cross-checking against 15+ rules…</div>
      </motion.div>
    );
  }
  // audit result
...
const AuditMessage = ({ result }: { result: AuditResult }) => {
  const map = {
      ...
      <div className="flex-1 space-y-4 min-w-0">
        ...
        <div className="rounded-2xl border border-border/60 gradient-card p-5 shadow-card">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="ml-auto flex gap-3 text-xs text-muted-foreground">
              <span><strong className="text-severity-high">{result.violations.filter((v) => v.severity === "High").length}</strong> High</span>
              <span><strong className="text-severity-medium">{result.violations.filter((v) => v.severity === "Medium").length}</strong> Medium</span>
              {!!result.contract_value_eur && (
                <span>€<strong className="text-foreground font-mono">{result.contract_value_eur.toLocaleString()}</strong></span>
              )}
            </div>
          </div>
        ...
        {result.escalation_required && (
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5">Per BMW Playbook P-02.</div>
            </div>
          </div>
        )}
        ...
        {result.violations.length > 0 ? (
          ...
          <div className="rounded-2xl border border-success/30 p-6 text-center" style={{ background: "hsl(var(--success) / 0.05)" }}>
            ...
            <div className="font-semibold">No violations detected</div>
          </div>
        )}
      </div>
    </motion.div>
